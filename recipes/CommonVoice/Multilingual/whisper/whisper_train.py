...
def get_parser() -> argparse.Namespace:
    ...
    # parser.add_argument('--dataset_dir', required=True, nargs='+', help='path to dataset directory')
    # parser.add_argument('--manifest_dir', required=False, nargs='+', help='path to manifest directory')
    parser.add_argument('--dataset_size', required=False, default='small',choices=['small', 'medium', 'full'])
    ...
    if not args.manifest_dir:
        args.manifest_dir = f"{Path(args.base_dir)}_{args.dataset_size}"
...
def main():
    ...
    manifests = load_manifests(args.dataset_size, args.base_dir, args.manifest_dir,args.lang, )
    ...
    if args.do_test:
        ...
        dataset = CommonVoiceDataset(manifest=manifests['test'], tokenizer=wtokenizer)
