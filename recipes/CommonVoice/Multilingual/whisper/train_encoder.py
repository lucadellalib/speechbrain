...
"""Recipe for fine-tuning Whisper encoder for ASR on Common Voice.
...
class ASR(sb.core.Brain):
    ...
    def fit_batch(self, batch):
        ...
        if self.auto_mix_prec:
            ...
            if not self.hparams.whisper.freeze:
                self.optimizer_whisper.zero_grad()
            ...
            if not self.hparams.whisper.freeze:
                self.scaler.unscale_(self.optimizer_whisper)
            self.scaler.unscale_(self.optimizer)

            if self.check_gradients(loss):
                if not self.hparams.whisper.freeze:
                    self.scaler.step(self.optimizer_whisper)
    ...
    def on_stage_end(self, stage, stage_loss, epoch):
        ...
        if stage == sb.Stage.VALID:
            ...
            (
                old_lr_whisper,
                new_lr_whisper,
            ) = self.hparams.lr_annealing_whisper(stage_stats["loss"])
            sb.nnet.schedulers.update_learning_rate(self.optimizer, new_lr)
            if not self.hparams.whisper.freeze:
                sb.nnet.schedulers.update_learning_rate(
                    self.optimizer_whisper, new_lr_whisper
                )
            stats_meta_data = {
                "epoch": epoch,
                "lr": old_lr,
    ...
    def init_optimizers(self):
        ...
        if not self.hparams.whisper.freeze:
            self.optimizer_whisper = self.hparams.opt_class_whisper(
                self.modules.whisper.parameters()
            )
            if self.checkpointer is not None:
                self.checkpointer.add_recoverable(
                    "optimizer_whisper", self.optimizer_whisper
                )
