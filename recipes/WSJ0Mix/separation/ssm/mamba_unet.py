...
class DilatedConvNorm(nn.Module):
    ...
    def __init__(self, nIn, nOut, kSize, stride=1, d=1, groups=1):
        ...
        self.mamba = Mamba(
            ...
            d_model=nOut,  # Model dimension d_model
    ...
    def forward(self, input):
        output = self.conv(input)
        output = self.mamba(output.movedim(-1, -2)).movedim(-1, -2)
        return self.norm(output)
...
if __name__ == "__main__":
    model = SuDORMRF(out_channels=64,
                     in_channels=64,
                     num_blocks=4,
                     upsampling_depth=4,
                     enc_kernel_size=21,
    ...
    # B x C x T
    dummy_input = torch.rand(3, 1, 32079).cuda()
